<script setup lang="ts">
import { ref, type Ref } from 'vue';


const detail: Ref<string | null> = ref(null)
</script>

<template>
    <div
        class="absolute top-0 right-0 z-50 flex flex-col items-center justify-start w-2/5 h-full bg-white border-l rounded-r-2xl">
        <header class="flex flex-row items-center justify-start w-full border-b">
            <div class="flex flex-row items-center px-6 py-4">
                <h4 class="text-sm font-bold">Add new task</h4>
            </div>

            <div class="flex flex-row ml-auto mr-4 space-x-4">
                <button>
                    <svg viewBox="0 0 24 24" class="w-6" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M7 12C7 13.1046 6.10457 14 5 14C3.89543 14 3 13.1046 3 12C3 10.8954 3.89543 10 5 10C6.10457 10 7 10.8954 7 12Z"
                            class="fill-gray-500"></path>
                        <path
                            d="M21 12C21 13.1046 20.1046 14 19 14C17.8954 14 17 13.1046 17 12C17 10.8954 17.8954 10 19 10C20.1046 10 21 10.8954 21 12Z"
                            class="fill-gray-500"></path>
                        <path
                            d="M14 12C14 13.1046 13.1046 14 12 14C10.8954 14 10 13.1046 10 12C10 10.8954 10.8954 10 12 10C13.1046 10 14 10.8954 14 12Z"
                            class="opacity-50 fill-gray-500"></path>
                    </svg>
                </button>

                <button>
                    <svg class="w-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" class="fill-gray-500"
                            d="M13.803 5.33333C13.803 3.49238 15.3022 2 17.1515 2C19.0008 2 20.5 3.49238 20.5 5.33333C20.5 7.17428 19.0008 8.66667 17.1515 8.66667C16.2177 8.66667 15.3738 8.28596 14.7671 7.67347L10.1317 10.8295C10.1745 11.0425 10.197 11.2625 10.197 11.4872C10.197 11.9322 10.109 12.3576 9.94959 12.7464L15.0323 16.0858C15.6092 15.6161 16.3473 15.3333 17.1515 15.3333C19.0008 15.3333 20.5 16.8257 20.5 18.6667C20.5 20.5076 19.0008 22 17.1515 22C15.3022 22 13.803 20.5076 13.803 18.6667C13.803 18.1845 13.9062 17.7255 14.0917 17.3111L9.05007 13.9987C8.46196 14.5098 7.6916 14.8205 6.84848 14.8205C4.99917 14.8205 3.5 13.3281 3.5 11.4872C3.5 9.64623 4.99917 8.15385 6.84848 8.15385C7.9119 8.15385 8.85853 8.64725 9.47145 9.41518L13.9639 6.35642C13.8594 6.03359 13.803 5.6896 13.803 5.33333Z">
                        </path>
                    </svg>
                </button>

                <button>
                    <svg class="w-5" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
                        <path class="fill-gray-500"
                            d="M195.2 195.2a64 64 0 0 1 90.496 0L512 421.504 738.304 195.2a64 64 0 0 1 90.496 90.496L602.496 512 828.8 738.304a64 64 0 0 1-90.496 90.496L512 602.496 285.696 828.8a64 64 0 0 1-90.496-90.496L421.504 512 195.2 285.696a64 64 0 0 1 0-90.496z">
                        </path>
                    </svg>
                </button>
            </div>
        </header>

        <main class="flex flex-col items-center justify-center w-full p-4">
            <input type="text" class="w-full text-3xl outline-none" placeholder="Story Title...">

            <div class="flex flex-row items-center w-full mt-4">
                <div class="flex flex-row items-center w-1/3">
                    <svg viewBox="0 0 24 24" class="w-6 mr-3" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                            d="M12 3.25C12.4142 3.25 12.75 3.58579 12.75 4V6.4C12.75 6.81421 12.4142 7.15 12 7.15C11.5858 7.15 11.25 6.81421 11.25 6.4V4C11.25 3.58579 11.5858 3.25 12 3.25ZM3.25 12C3.25 11.5858 3.58579 11.25 4 11.25H6.4C6.81421 11.25 7.15 11.5858 7.15 12C7.15 12.4142 6.81421 12.75 6.4 12.75H4C3.58579 12.75 3.25 12.4142 3.25 12ZM16.85 12C16.85 11.5858 17.1858 11.25 17.6 11.25H20C20.4142 11.25 20.75 11.5858 20.75 12C20.75 12.4142 20.4142 12.75 20 12.75H17.6C17.1858 12.75 16.85 12.4142 16.85 12ZM12 16.85C12.4142 16.85 12.75 17.1858 12.75 17.6V20C12.75 20.4142 12.4142 20.75 12 20.75C11.5858 20.75 11.25 20.4142 11.25 20V17.6C11.25 17.1858 11.5858 16.85 12 16.85Z"
                            class="fill-gray-500"></path>
                        <g class="opacity-50">
                            <path
                                d="M18.187 5.81244C18.4799 6.10534 18.4799 6.58021 18.187 6.8731L16.49 8.57016C16.1971 8.86305 15.7222 8.86305 15.4293 8.57016C15.1364 8.27727 15.1364 7.80239 15.4293 7.5095L17.1264 5.81244C17.4193 5.51955 17.8941 5.51955 18.187 5.81244Z"
                                class="fill-gray-500"></path>
                            <path
                                d="M5.81244 5.81262C6.10534 5.51973 6.58021 5.51973 6.8731 5.81262L8.57016 7.50968C8.86305 7.80257 8.86305 8.27744 8.57016 8.57034C8.27727 8.86323 7.80239 8.86323 7.5095 8.57034L5.81244 6.87328C5.51955 6.58039 5.51955 6.10551 5.81244 5.81262Z"
                                class="fill-gray-500"></path>
                            <path
                                d="M8.57037 15.4291C8.86327 15.722 8.86327 16.1969 8.57037 16.4898L6.87332 18.1868C6.58042 18.4797 6.10555 18.4797 5.81266 18.1868C5.51976 17.8939 5.51976 17.419 5.81266 17.1262L7.50971 15.4291C7.80261 15.1362 8.27748 15.1362 8.57037 15.4291Z"
                                class="fill-gray-500"></path>
                            <path
                                d="M15.4291 15.4293C15.722 15.1364 16.1969 15.1364 16.4898 15.4293L18.1868 17.1263C18.4797 17.4192 18.4797 17.8941 18.1868 18.187C17.8939 18.4799 17.419 18.4799 17.1262 18.187L15.4291 16.4899C15.1362 16.197 15.1362 15.7222 15.4291 15.4293Z"
                                class="fill-gray-500"></path>
                        </g>
                    </svg>
                    <span class="text-sm font-medium text-gray-500">Status</span>
                </div>

                <div class="flex flex-row items-center w-2/3">
                    <span class="px-2 py-1 text-xs font-medium text-blue-500 bg-blue-100 rounded-lg">Not
                        Started</span>
                </div>
            </div>

            <div class="flex flex-row items-center w-full mt-4">
                <div class="flex flex-row items-center w-1/3">
                    <svg viewBox="0 0 24 24" class="w-6 mr-3" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M6.96006 2C7.37758 2 7.71605 2.30996 7.71605 2.69231V4.08883C8.38663 4.07692 9.13829 4.07692 9.98402 4.07692H14.016C14.8617 4.07692 15.6134 4.07692 16.284 4.08883V2.69231C16.284 2.30996 16.6224 2 17.0399 2C17.4575 2 17.7959 2.30996 17.7959 2.69231V4.15008C19.2468 4.25647 20.1992 4.51758 20.899 5.15838C21.5987 5.79917 21.8838 6.67139 22 8V9H2V8C2.11618 6.67139 2.4013 5.79917 3.10104 5.15838C3.80079 4.51758 4.75323 4.25647 6.20406 4.15008V2.69231C6.20406 2.30996 6.54253 2 6.96006 2Z"
                            class="fill-gray-500"></path>
                        <path opacity="0.5"
                            d="M22 14V12C22 11.161 21.9873 9.66527 21.9744 9H2.00586C1.99296 9.66527 2.00564 11.161 2.00564 12V14C2.00564 17.7712 2.00564 19.6569 3.17688 20.8284C4.34813 22 6.23321 22 10.0034 22H14.0023C17.7724 22 19.6575 22 20.8288 20.8284C22 19.6569 22 17.7712 22 14Z"
                            class="fill-gray-500"></path>
                        <path fill-rule="evenodd" clip-rule="evenodd"
                            d="M14 12.25C13.0335 12.25 12.25 13.0335 12.25 14V16C12.25 16.9665 13.0335 17.75 14 17.75C14.9665 17.75 15.75 16.9665 15.75 16V14C15.75 13.0335 14.9665 12.25 14 12.25ZM14 13.75C13.8619 13.75 13.75 13.862 13.75 14V16C13.75 16.1381 13.8619 16.25 14 16.25C14.1381 16.25 14.25 16.1381 14.25 16V14C14.25 13.862 14.1381 13.75 14 13.75Z"
                            class="fill-gray-500"></path>
                        <path
                            d="M11.25 13C11.25 12.6967 11.0673 12.4232 10.787 12.3071C10.5068 12.191 10.1842 12.2552 9.96967 12.4697L8.46967 13.9697C8.17678 14.2626 8.17678 14.7375 8.46967 15.0304C8.76256 15.3232 9.23744 15.3232 9.53033 15.0304L9.75 14.8107V17C9.75 17.4142 10.0858 17.75 10.5 17.75C10.9142 17.75 11.25 17.4142 11.25 17V13Z"
                            class="fill-gray-500"></path>
                    </svg>
                    <span class="text-sm font-medium text-gray-500">Due Date</span>
                </div>

                <div class="flex flex-row items-center w-2/3">
                    <span class="px-2 py-1 text-sm font-medium ">2024/05/10</span>
                </div>
            </div>

            <div class="flex flex-row items-center w-full mt-4">
                <div class="flex flex-row items-center w-1/3">
                    <svg viewBox="0 0 24 24" class="w-6 mr-3" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                            d="M21.3023 13.1628C21.6876 13.1628 22 13.4752 22 13.8605V13.9129C22 15.6225 22 16.9766 21.8575 18.0363C21.7109 19.1269 21.402 20.0097 20.7058 20.7058C20.0097 21.402 19.1269 21.7109 18.0363 21.8575C16.9766 22 15.6225 22 13.9129 22H13.8605C13.4752 22 13.1628 21.6876 13.1628 21.3023C13.1628 20.917 13.4752 20.6047 13.8605 20.6047C15.6343 20.6047 16.8944 20.6032 17.8504 20.4746C18.7863 20.3488 19.3255 20.1128 19.7192 19.7192C20.1128 19.3255 20.3488 18.7863 20.4746 17.8504C20.6032 16.8944 20.6047 15.6343 20.6047 13.8605C20.6047 13.4752 20.917 13.1628 21.3023 13.1628Z"
                            class="fill-gray-500"></path>
                        <path fill-rule="evenodd" clip-rule="evenodd"
                            d="M2.69768 13.1628C3.08299 13.1628 3.39535 13.4752 3.39535 13.8605C3.39535 15.6343 3.39683 16.8944 3.52536 17.8504C3.65119 18.7863 3.88716 19.3255 4.28084 19.7192C4.67452 20.1128 5.21373 20.3488 6.14963 20.4746C7.10559 20.6032 8.36575 20.6047 10.1395 20.6047C10.5248 20.6047 10.8372 20.917 10.8372 21.3023C10.8372 21.6876 10.5248 22 10.1395 22H10.0871C8.37751 22 7.02344 22 5.9637 21.8575C4.87307 21.7109 3.99033 21.402 3.29418 20.7058C2.59803 20.0097 2.28908 19.1269 2.14245 18.0363C1.99997 16.9766 1.99999 15.6225 2 13.9129C2 13.8955 2 13.878 2 13.8605C2 13.4752 2.31236 13.1628 2.69768 13.1628Z"
                            class="fill-gray-500"></path>
                        <path fill-rule="evenodd" clip-rule="evenodd"
                            d="M10.0871 2L10.1395 2C10.5249 2 10.8372 2.31236 10.8372 2.69768C10.8372 3.08299 10.5249 3.39535 10.1395 3.39535C8.36575 3.39535 7.10559 3.39683 6.14963 3.52536C5.21373 3.65119 4.67452 3.88716 4.28084 4.28084C3.88716 4.67452 3.65119 5.21373 3.52536 6.14963C3.39683 7.10559 3.39535 8.36575 3.39535 10.1395C3.39535 10.5248 3.08299 10.8372 2.69768 10.8372C2.31236 10.8372 2 10.5248 2 10.1395L2 10.0871C1.99999 8.37751 1.99997 7.02343 2.14245 5.9637C2.28908 4.87307 2.59803 3.99033 3.29418 3.29418C3.99033 2.59803 4.87308 2.28908 5.9637 2.14245C7.02343 1.99997 8.37751 1.99999 10.0871 2Z"
                            class="fill-gray-500"></path>
                        <path fill-rule="evenodd" clip-rule="evenodd"
                            d="M17.8504 3.52536C16.8944 3.39683 15.6343 3.39535 13.8605 3.39535C13.4752 3.39535 13.1628 3.08299 13.1628 2.69768C13.1628 2.31236 13.4752 2 13.8605 2C13.878 2 13.8955 2 13.9129 2C15.6225 1.99999 16.9766 1.99997 18.0363 2.14245C19.1269 2.28908 20.0097 2.59803 20.7058 3.29418C21.402 3.99033 21.7109 4.87307 21.8575 5.9637C22 7.02344 22 8.37751 22 10.0871V10.1395C22 10.5249 21.6876 10.8372 21.3023 10.8372C20.917 10.8372 20.6047 10.5249 20.6047 10.1395C20.6047 8.36575 20.6032 7.10559 20.4746 6.14963C20.3488 5.21373 20.1128 4.67452 19.7192 4.28084C19.3255 3.88716 18.7863 3.65119 17.8504 3.52536Z"
                            class="fill-gray-500"></path>
                        <path
                            d="M10.373 10.4309C10.373 11.1052 10.0086 11.6518 9.55907 11.6518C9.10954 11.6518 8.74512 11.1052 8.74512 10.4309C8.74512 9.75659 9.10954 9.20996 9.55907 9.20996C10.0086 9.20996 10.373 9.75659 10.373 10.4309Z"
                            class="fill-gray-500"></path>
                        <path
                            d="M15.2567 10.4309C15.2567 11.1052 14.8923 11.6518 14.4428 11.6518C13.9933 11.6518 13.6288 11.1052 13.6288 10.4309C13.6288 9.75659 13.9933 9.20996 14.4428 9.20996C14.8923 9.20996 15.2567 9.75659 15.2567 10.4309Z"
                            class="fill-gray-500"></path>
                        <path
                            d="M9.92259 14.4172C9.65174 14.2164 9.26941 14.2733 9.06864 14.5441C8.86787 14.815 8.92469 15.1973 9.19555 15.3981C9.98705 15.9847 10.955 16.3321 12.0009 16.3321C13.0468 16.3321 14.0148 15.9847 14.8063 15.3981C15.0772 15.1973 15.134 14.815 14.9332 14.5441C14.7325 14.2733 14.3501 14.2164 14.0793 14.4172C13.4864 14.8566 12.7708 15.1111 12.0009 15.1111C11.231 15.1111 10.5154 14.8566 9.92259 14.4172Z"
                            class="fill-gray-500"></path>
                        <g class="opacity-50">
                            <path
                                d="M10.3725 10.4307C10.3725 11.105 10.0081 11.6516 9.55859 11.6516C9.10906 11.6516 8.74464 11.105 8.74464 10.4307C8.74464 9.75639 9.10906 9.20976 9.55859 9.20976C10.0081 9.20976 10.3725 9.75639 10.3725 10.4307Z"
                                class="fill-gray-500"></path>
                            <path
                                d="M15.2563 10.4307C15.2563 11.105 14.8918 11.6516 14.4423 11.6516C13.9928 11.6516 13.6284 11.105 13.6284 10.4307C13.6284 9.75639 13.9928 9.20976 14.4423 9.20976C14.8918 9.20976 15.2563 9.75639 15.2563 10.4307Z"
                                class="fill-gray-500"></path>
                            <path
                                d="M9.92212 14.417C9.65126 14.2162 9.26893 14.2731 9.06817 14.5439C8.8674 14.8148 8.92422 15.1971 9.19507 15.3979C9.98657 15.9845 10.9545 16.3319 12.0005 16.3319C13.0464 16.3319 14.0143 15.9845 14.8058 15.3979C15.0767 15.1971 15.1335 14.8148 14.9327 14.5439C14.732 14.2731 14.3496 14.2162 14.0788 14.417C13.486 14.8564 12.7704 15.1109 12.0005 15.1109C11.2305 15.1109 10.5149 14.8564 9.92212 14.417Z"
                                class="fill-gray-500"></path>
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M10.14 4.55859H13.8609C16.492 4.55859 17.8076 4.55859 18.6249 5.37597C19.4423 6.19335 19.4423 7.50889 19.4423 10.14V13.8609C19.4423 16.492 19.4423 17.8076 18.6249 18.6249C17.8076 19.4423 16.492 19.4423 13.8609 19.4423H10.14C7.50889 19.4423 6.19335 19.4423 5.37597 18.6249C4.55859 17.8076 4.55859 16.492 4.55859 13.8609V10.14C4.55859 7.50889 4.55859 6.19335 5.37597 5.37597C6.19335 4.55859 7.50889 4.55859 10.14 4.55859ZM9.06817 14.5439C9.26893 14.2731 9.65126 14.2162 9.92212 14.417C10.5149 14.8564 11.2305 15.1109 12.0005 15.1109C12.7704 15.1109 13.486 14.8564 14.0788 14.417C14.3496 14.2162 14.732 14.2731 14.9327 14.5439C15.1335 14.8148 15.0767 15.1971 14.8058 15.3979C14.0143 15.9845 13.0464 16.3319 12.0005 16.3319C10.9545 16.3319 9.98657 15.9845 9.19507 15.3979C8.92422 15.1971 8.8674 14.8148 9.06817 14.5439ZM14.4423 11.6516C14.8918 11.6516 15.2563 11.105 15.2563 10.4307C15.2563 9.75639 14.8918 9.20976 14.4423 9.20976C13.9928 9.20976 13.6284 9.75639 13.6284 10.4307C13.6284 11.105 13.9928 11.6516 14.4423 11.6516ZM9.55859 11.6516C10.0081 11.6516 10.3725 11.105 10.3725 10.4307C10.3725 9.75639 10.0081 9.20976 9.55859 9.20976C9.10906 9.20976 8.74464 9.75639 8.74464 10.4307C8.74464 11.105 9.10906 11.6516 9.55859 11.6516Z"
                                class="fill-gray-500"></path>
                        </g>
                    </svg>
                    <span class="text-sm font-medium text-gray-500">Assignees</span>
                </div>

                <div class="flex flex-row items-center w-2/3">
                    <div class="flex flex-row items-center px-2 py-1 text-sm">
                        <div class="w-6 h-6 mr-2 bg-gray-700 rounded-full aspect-square"></div>
                        <span>Hossein Nodehi</span>
                    </div>
                </div>
            </div>

            <div class="flex flex-row items-center w-full mt-4">
                <div class="flex flex-row items-center w-1/3">
                    <svg viewBox="0 0 24 24" class="w-6 mr-3" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M4.72848 16.1369C3.18295 14.5914 2.41018 13.8186 2.12264 12.816C1.83509 11.8134 2.08083 10.7485 2.57231 8.61875L2.85574 7.39057C3.26922 5.59881 3.47597 4.70292 4.08944 4.08944C4.70292 3.47597 5.5988 3.26922 7.39057 2.85574L8.61875 2.57231C10.7485 2.08083 11.8134 1.83509 12.816 2.12264C13.8186 2.41018 14.5914 3.18295 16.1369 4.72848L17.9665 6.55812L17.9665 6.55813C20.6555 9.24711 22 10.5916 22 12.2623C22 13.933 20.6555 15.2775 17.9665 17.9665L17.9665 17.9665L17.9665 17.9665C15.2775 20.6555 13.933 22 12.2623 22C10.5916 22 9.24711 20.6555 6.55813 17.9665L6.55812 17.9665L4.72848 16.1369Z"
                            class="opacity-50 fill-gray-500"></path>
                        <path
                            d="M10.1235 7.27135C10.911 8.05894 10.911 9.33587 10.1235 10.1235C9.33587 10.911 8.05894 10.911 7.27135 10.1235C6.48377 9.33587 6.48377 8.05894 7.27135 7.27135C8.05894 6.48377 9.33587 6.48377 10.1235 7.27135Z"
                            class="fill-gray-500"></path>
                        <path
                            d="M19.0512 12.0514L12.0721 19.0307C11.7793 19.3236 11.3044 19.3236 11.0115 19.0307C10.7186 18.7378 10.7186 18.263 11.0115 17.9701L17.9905 10.9908C18.2834 10.6979 18.7582 10.6979 19.0511 10.9908C19.344 11.2837 19.344 11.7586 19.0512 12.0514Z"
                            class="fill-gray-500"></path>
                    </svg>
                    <span class="text-sm font-medium text-gray-500">Tags</span>
                </div>

                <div class="flex flex-row items-center w-2/3">
                    <span class="px-2 py-1 mr-2 text-xs font-medium text-red-500 bg-red-100 rounded-lg">Dev</span>
                    <span class="px-2 py-1 text-xs font-medium text-orange-500 bg-orange-100 rounded-lg">Tech</span>
                </div>
            </div>

            <div class="flex flex-col items-center w-full mt-4">
                <div class="flex flex-row items-center w-full">
                    <svg viewBox="0 0 24 24" class="w-6 mr-3" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                            d="M4 4.69434V18.6943C4 20.3512 5.34315 21.6943 7 21.6943H17C18.6569 21.6943 20 20.3512 20 18.6943V8.69434C20 7.03748 18.6569 5.69434 17 5.69434H5C4.44772 5.69434 4 5.24662 4 4.69434ZM7.25 11.6943C7.25 11.2801 7.58579 10.9443 8 10.9443H16C16.4142 10.9443 16.75 11.2801 16.75 11.6943C16.75 12.1085 16.4142 12.4443 16 12.4443H8C7.58579 12.4443 7.25 12.1085 7.25 11.6943ZM7.25 15.1943C7.25 14.7801 7.58579 14.4443 8 14.4443H13.5C13.9142 14.4443 14.25 14.7801 14.25 15.1943C14.25 15.6085 13.9142 15.9443 13.5 15.9443H8C7.58579 15.9443 7.25 15.6085 7.25 15.1943Z"
                            class="fill-gray-500"></path>
                        <path
                            d="M18 4.00038V5.86504C17.6872 5.75449 17.3506 5.69434 17 5.69434H5C4.44772 5.69434 4 5.24662 4 4.69434V4.62329C4 4.09027 4.39193 3.63837 4.91959 3.56299L15.7172 2.02048C16.922 1.84835 18 2.78328 18 4.00038Z"
                            class="opacity-50 fill-gray-500"></path>
                    </svg>
                    <span class="text-sm font-medium text-gray-500">Details</span>
                </div>

                <div v-if="detail == null"
                    class="flex flex-row items-center justify-center w-full mt-4 space-x-4 border-2 border-gray-300 border-dashed rounded-xl min-h-32">
                    <button @click="detail = 'description'"
                        class="px-2 py-1 text-sm text-white bg-blue-500 rounded-lg">Description</button>
                    <button @click="detail = 'story'" class="px-2 py-1 text-sm text-white bg-blue-500 rounded-lg">User
                        Story</button>
                </div>

                <div v-if="detail == 'story'" class="flex flex-col w-full mt-4 ml-4">
                    <div class="flex flex-row items-center space-x-2">
                        <h6>As a </h6>
                        <select class="outline-none">
                            <option value="">User</option>
                            <option value="">Purchaser</option>
                        </select>
                    </div>

                    <div class="flex flex-row items-center space-x-2">
                        <h6>I want to</h6>
                        <input type="text" class="outline-none" placeholder="do what?">
                    </div>

                    <div class="flex flex-row items-center space-x-2">
                        <h6>So that </h6>
                        <input type="text" class="outline-none" placeholder="for what?" />
                    </div>
                </div>

                <div v-if="detail == 'description'" class="w-full mt-4">
                    <textarea class="w-full outline-none" placeholder="Desctiption"></textarea>
                </div>
            </div>

            <div class="flex flex-col items-center w-full mt-4">
                <div class="flex flex-row items-center w-full">
                    <svg viewBox="0 0 24 24" class="w-6 mr-3" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                            d="M11.2442 1.95482C12.9441 1.01506 15.0345 1.01506 16.7345 1.95482C17.3641 2.30291 17.9518 2.86575 18.9065 3.78014C18.9373 3.80965 18.9685 3.83952 19.0001 3.86977C19.2993 4.15623 19.3096 4.631 19.0231 4.93018C18.7366 5.22936 18.2619 5.23967 17.9627 4.9532C16.8844 3.92069 16.4452 3.50886 16.0087 3.26758C14.7604 2.57747 13.2183 2.57747 11.9699 3.26758C11.5334 3.50886 11.0943 3.92069 10.0159 4.9532L4.02651 10.6881C3.72732 10.9745 3.25256 10.9642 2.9661 10.665C2.67963 10.3659 2.68994 9.8911 2.98912 9.60464L8.97855 3.86977C9.01014 3.83952 9.04133 3.80965 9.07214 3.78014C10.0268 2.86575 10.6145 2.30291 11.2442 1.95482ZM14.945 6.76023C15.2314 6.46104 15.7062 6.45074 16.0054 6.7372C16.0326 6.76328 16.0596 6.78906 16.0863 6.81457C16.4533 7.16504 16.7689 7.46639 16.9459 7.81825C17.2622 8.44722 17.2622 9.18288 16.9459 9.81184C16.7689 10.1637 16.4533 10.4651 16.0863 10.8155C16.0596 10.841 16.0326 10.8668 16.0054 10.8929L8.62553 17.9591C8.32635 18.2455 7.85159 18.2352 7.56512 17.936C7.27866 17.6369 7.28897 17.1621 7.58815 16.8756L14.968 9.80945C15.4628 9.33562 15.5615 9.22595 15.6058 9.13786C15.7089 8.93291 15.7089 8.69718 15.6058 8.49224C15.5615 8.40415 15.4628 8.29447 14.968 7.82064C14.6688 7.53417 14.6585 7.05941 14.945 6.76023Z"
                            class="fill-gray-500"></path>
                        <path
                            d="M17.9628 4.95358C19.0429 5.9878 19.4703 6.40622 19.7194 6.81926C20.4269 7.99256 20.4269 9.43347 19.7194 10.6068C19.4703 11.0198 19.0429 11.4382 17.9628 12.4725L10.5295 19.5898C9.97082 20.1247 9.58528 20.493 9.26135 20.7543C8.94539 21.0092 8.7384 21.1195 8.55399 21.1725C8.19285 21.2763 7.80709 21.2763 7.44595 21.1725C7.26154 21.1195 7.05455 21.0092 6.73859 20.7543C6.41466 20.493 6.02912 20.1247 5.47047 19.5898C4.91163 19.0547 4.52727 18.6858 4.2548 18.3761C3.98794 18.0728 3.87862 17.8805 3.82747 17.7174C3.72418 17.388 3.72418 17.0378 3.82747 16.7084C3.87862 16.5453 3.98794 16.353 4.2548 16.0497C4.52727 15.74 4.91163 15.3711 5.47047 14.836L12.7968 7.82101C13.2887 7.35 13.4068 7.25113 13.5074 7.20474C13.7436 7.09583 14.0213 7.09583 14.2575 7.20474C14.3581 7.25113 14.4761 7.35 14.9681 7.82101C14.9682 7.82113 14.9679 7.8209 14.9681 7.82101C14.6689 7.53455 14.6585 7.05916 14.945 6.75998C15.2259 6.46658 15.6879 6.45099 15.9878 6.72068L15.9274 6.66272C15.558 6.30819 15.2461 6.00881 14.8856 5.84257C14.2508 5.54989 13.5141 5.54989 12.8793 5.84257C12.5188 6.00881 12.2069 6.30819 11.8375 6.66271L4.40899 13.7756C3.88009 14.282 3.44862 14.6951 3.12862 15.0589C2.79815 15.4345 2.53687 15.811 2.3962 16.2596C2.20127 16.8811 2.20127 17.5447 2.3962 18.1663C2.53687 18.6148 2.79815 18.9913 3.12862 19.3669C3.44862 19.7307 3.88006 20.1438 4.40896 20.6502L4.4568 20.696C4.98604 21.2027 5.41736 21.6157 5.79685 21.9218C6.18996 22.2389 6.57706 22.4834 7.03145 22.6141C7.66343 22.7958 8.3365 22.7958 8.96849 22.6141C9.42288 22.4834 9.80998 22.2389 10.2031 21.9218C10.5826 21.6157 11.0139 21.2028 11.5431 20.696L19.0979 13.4623C20.0489 12.552 20.6381 11.988 21.0039 11.3813C21.9987 9.73161 21.9987 7.69442 21.0039 6.04471C20.6381 5.43798 20.0489 4.874 19.0979 3.96371L19.0175 3.88672C19.2996 4.17468 19.3039 4.63666 19.0231 4.92993C18.7366 5.22911 18.262 5.24004 17.9628 4.95358Z"
                            class="opacity-50 fill-gray-500"></path>
                    </svg>
                    <span class="text-sm font-medium text-gray-500">Attachments</span>
                </div>

                <div class="flex flex-row items-center w-full mt-4 space-x-4">
                    <div class="flex flex-row items-center border rounded-lg">
                        <div class="p-2 border-r">
                            <div class="p-4 rounded-full aspect-square bg-emerald-100 text-emerald-500">ZIP</div>
                        </div>

                        <div class="flex flex-col items-center w-full px-4">
                            <p class="w-full">Filename is here</p>
                            <p class="w-full text-xs">24 MB</p>
                        </div>
                    </div>
                </div>
            </div>

            <div class="flex flex-row items-center w-full p-1 mt-4 space-x-1 bg-gray-200 rounded-full">
                <button class="w-1/2 px-2 py-1 text-sm font-medium bg-gray-300 rounded-full">Comments</button>
                <button class="w-1/2 px-2 py-1 text-sm font-medium rounded-full">Activities</button>
            </div>
        </main>
    </div>
</template>